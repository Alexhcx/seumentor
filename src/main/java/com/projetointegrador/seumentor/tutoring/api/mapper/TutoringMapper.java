...
@Mapper(componentModel = "spring", imports = { DateTimeFormatter.class, Collectors.class, TutoringClassType.class,
        StatusTutoring.class, User.class, Discipline.class, ArrayList.class, LoggerFactory.class, Collections.class,
        Optional.class, CourseArea.class, DayWeek.class }, unmappedTargetPolicy = ReportingPolicy.WARN)
public abstract class TutoringMapper {
    ...
    @Mapping(target = "endTime", source = "endTime", qualifiedByName = "formatTimeSafe")
    @Mapping(target = "tutoringDate", source = "tutoringDate", qualifiedByName = "formatDateSafe")
    @Mapping(target = "isMentorPostingOnly", source = "isMentorPostingOnly") 
    ...
    public abstract List<UserAvailabilityRepresentation> toUserAvailabilityRepresentationList(
    ...
    public abstract TutoringRatingRepresentation toTutoringRatingRepresentation(TutoringRating rating,
            @Context Long raterUserId);
    // Adicionado @Context para raterUserId se ele não for um campo direto de
    // TutoringRating
    // Se raterUserId não for usado para mapear nenhum campo em
    // TutoringRatingRepresentation, pode ser removido do método.
    // Assumindo que o DTO tem um campo para raterUserId que será preenchido
    // diretamente.
    // Se `TutoringRatingRepresentation` tem um campo `ratedByUserId`, e
    // `raterUserId` é para ele:
    // @Mapping(target = "ratedByUserId", source = "raterUserId") -> se raterUserId
    ...
    protected String userToFullName(User user) {
        if (user == null)
            return "[Usuário N/A]"; // Ajustado para ser mais genérico
        try {
            ...
            log.warn("Não foi possível obter o nome completo do usuário ID: {}",
                    (user.getId() != null ? user.getId() : "null"), e);
    ...
    protected String disciplineToName(Discipline discipline) {
        if (discipline == null)
            return "[Disciplina N/A]";
        try {
            if (discipline.getDisciplineName() == null && discipline.getId() != null) { // Heurística para proxy
                log.warn("Tentando acessar nome de um proxy de Discipline não totalmente carregado. ID: {}",
                        discipline.getId());
                return "[Disciplina Carregando...]";
            }
            return discipline.getDisciplineName();
        } catch (Exception e) {
            log.warn("Não foi possível obter o nome da disciplina ID: {}",
                    (discipline.getId() != null ? discipline.getId() : "null"), e);
    ...
    protected SimpleDisciplineRepresentation disciplineToSimpleDisciplineRepSafe(Discipline discipline) {
        ...
        try {
            ...
            log.warn("Disciplina (proxy) não encontrada ao mapear para SimpleDisciplineRepresentation. ID pode ser {}.",
                    (discipline.getId() != null ? discipline.getId() : "desconhecido"), e);
            return new SimpleDisciplineRepresentation(null, "[Disciplina Inválida/Removida]");
        } catch (Exception e) {
            log.warn(
                    "Erro ao acessar dados da disciplina ao mapear para SimpleDisciplineRepresentation. ID pode ser {}.",
                    (discipline.getId() != null ? discipline.getId() : "desconhecido"), e);
    ...
    public Optional<TutoringRepresentation> availabilityToTutoringRepresentation(MentorAvailability availability,
            LocalDate date) {
        ...
        if (mentor == null || discipline == null || date == null) {
            log.warn(
                    "Skipping mapping availability ID {} to TutoringRepresentation due to null User, Discipline or Date.",
                    (availability.getId() != null ? availability.getId() : "N/A"));
            return Optional.empty();
        }
    ...
    // Para (Discipline, List<MentorAvailability>) ->
    // MentorDisciplineAvailabilityRepresentation
    @Named("toMentorDisciplineAvailabilityRep")
    public MentorDisciplineAvailabilityRepresentation toMentorDisciplineAvailabilityRepresentation(
            Discipline discipline, List<MentorAvailability> availabilities) {
        if (discipline == null)
            return null;
    ...
    // Renomeado para evitar conflito de nome se um método abstrato fosse gerado com
    // a mesma assinatura
