...
public class UserController {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "401", description = "Não autorizado", content = @Content),
            // @ApiResponse(responseCode = "403", description = "Acesso negado", content = @Content), // Redundant if isAuthenticated() passes
            @ApiResponse(responseCode = "500", description = "Erro interno no servidor", content = @Content)
    })
    ...
    public ResponseEntity<List<TutoringRepresentation>> getUserMentoringSessions(
        ...
        try {
            List<TutoringRepresentation> sessions = userQuery.getUserMentoringSessions(id);
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "401", description = "Não autorizado", content = @Content),
            // @ApiResponse(responseCode = "403", description = "Acesso negado", content = @Content), // Redundant
            @ApiResponse(responseCode = "500", description = "Erro interno no servidor", content = @Content)
    })
    ...
    public ResponseEntity<Void> addFavoriteDiscipline(
        ...
        try {
            ...
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, e.getMessage(), e);
        } catch (IllegalArgumentException e) { // Catch conflict (already favorite) if service throws it
            log.warn("Controller: Add favorite discipline failed for user {}: {}", userId, e.getMessage());
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Senha alterada com sucesso",
                    content = @Content(mediaType = "application/json", schema = @Schema(type = "string", example = "Senha alterada com sucesso."))),
            @ApiResponse(responseCode = "400", description = "Requisição inválida (ex: nova senha curta, senhas iguais, senha antiga incorreta)",
                    content = @Content(mediaType = "application/json", schema = @Schema(type = "string"))),
            @ApiResponse(responseCode = "401", description = "Não autorizado", content = @Content),
            @ApiResponse(responseCode = "403", description = "Acesso negado (tentando alterar senha de outro usuário sem ser ADMIN)", content = @Content),
            @ApiResponse(responseCode = "404", description = "Usuário não encontrado", content = @Content),
            @ApiResponse(responseCode = "500", description = "Erro interno no servidor", content = @Content)
    })
    public ResponseEntity<?> changePassword(
            @Parameter(description = "ID do usuário cuja senha será alterada", required = true)
            @PathVariable Long id,
            @RequestBody(description = "Senha antiga e nova senha", required = true,
        ...
        try {
            ...
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, e.getMessage(), e);
        } catch (BadCredentialsException | IllegalArgumentException e) { // Group bad request exceptions
            ...
            throw new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Erro ao processar a alteração de senha.", e);
        }
    }
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Status da disponibilidade atualizado com sucesso. Retorna a lista completa e atualizada de disponibilidades do usuário.",
                    content = @Content(mediaType = "application/json",
                            array = @ArraySchema(schema = @Schema(implementation = UserAvailabilityRepresentation.class)))),
            @ApiResponse(responseCode = "404", description = "Disponibilidade não encontrada ou não pertence ao usuário", content = @Content),
            @ApiResponse(responseCode = "401", description = "Não autorizado", content = @Content),
            @ApiResponse(responseCode = "403", description = "Acesso negado (usuário não é o dono nem ADMIN)", content = @Content),
            @ApiResponse(responseCode = "500", description = "Erro interno no servidor", content = @Content)
    })
    public ResponseEntity<List<UserAvailabilityRepresentation>> updateAvailabilityStatus(
        ...
        log.info("Controller: Request to update status for availability ID: {} for user ID: {} to {} (Authorized)", availabilityId, userId, request.isAvailable());
        try {
            List<UserAvailabilityRepresentation> updatedAvailabilities = userCommandService.updateAvailabilityStatus(userId, availabilityId, request);
    ...
    public ResponseEntity<Void> deleteAvailability(
        ...
        try {
            ...
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, e.getMessage(), e);
        } catch (AccessDeniedException e) { // Catch if service throws it
